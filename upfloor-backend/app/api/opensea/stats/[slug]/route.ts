...
/**
 * Fast stats endpoint for OpenSea collections - fetches directly from database cache
 * No external API calls, extremely fast response
 * Works for all chains (Ethereum, Polygon, etc.)
 */
export async function GET(
  ...
  try {
    ...
    // Works for all chains with OpenSea
    const statsQuery = `
      SELECT 
        total_supply, 
        listed_count, 
    ...
    const result = await pool.query(statsQuery, [slug]);
    ...
    console.error("OpenSea stats fetch error:", error);
    return NextResponse.json(
