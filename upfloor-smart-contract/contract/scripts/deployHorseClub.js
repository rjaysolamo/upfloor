/**
 * Horse Club Token Deployment Script
 * 
 * Deploys a new UpFloorToken for Horse Club collection using the Factory
 * 
 * Usage:
 *   npx hardhat run scripts/deployHorseClub.js --network <network-name>
 */
...
const CONFIG = {
    ...
    // Token configuration
    tokenName: "Horse Club Token",
    tokenSymbol: "HORSE",
    
    // NFT collection address
    nftCollectionAddress: "0x9d1E755ed23de4e07F8E9Da17cC05457475Ea17D",
    
    // Collection owner address (receives royalties) - YOUR ADDRESS
    collectionOwnerAddress: "0x42d4B976526035d682fF29BC791081e807aFdcD6",
    
    // Royalty percentage in basis points (300 = 3%)
    royaltyBps: 300,
...
async function main() {
    console.log("\n========================================");
    console.log("🐎 Horse Club Token Deployment");
    console.log("========================================\n");
    ...
    console.log("Royalty BPS:", CONFIG.royaltyBps, "(3%)");
    ...
    console.log("🚀 Deploying Horse Club token system via factory...");
    ...
    const tx = await factory.deployStrategyToken(
        ...
        CONFIG.collectionOwnerAddress, // royalty recipient (same as collection owner)
        CONFIG.royaltyBps,
    ...
    const deploymentInfo = {
        ...
        royaltyRecipient: CONFIG.collectionOwnerAddress,
    ...
    const filename = `${hre.network.name}-horse-deployment.json`;
    ...
    if (hre.network.name !== "hardhat" && hre.network.name !== "localhost") {
        ...
        console.log("─────────────────────────────────────");
        console.log("\n# Verify Token:");
        console.log(`npx hardhat verify --network ${hre.network.name} ${tokenAddress} "${CONFIG.tokenName}" "${CONFIG.tokenSymbol}" "${await token.getProtocolFeeRecipient()}" "${CONFIG.nftCollectionAddress}" "${CONFIG.collectionOwnerAddress}" "${CONFIG.collectionOwnerAddress}" "${CONFIG.royaltyBps}"`);
        console.log("\n# Verify Router:");
    ...
    console.log("5. 💎 3% royalties will go to:", CONFIG.collectionOwnerAddress);
    console.log();

    console.log("========================================");
    console.log("✅ Horse Club Token Deployment Complete!");
    console.log("========================================\n");
